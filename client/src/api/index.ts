...
import type { Plant, PlantAction, PlantActionRequest, PlantRequest } from '../types/plant.types';
...
export const plantsApi = {
  getPlants: async (page?: number, limit?: number, name?: string): Promise<PaginatedResponse<Plant>> => {
    const res = await api.get<PaginatedResponse<Plant>>('/plants', {params: {page, limit, name}});
  ...
  updatePlant: async (id: number, plant: PlantRequest): Promise<Plant> => {
    const res = await api.put<PlantMutationResponse>(`/updatePlant/${id}`, plant);
    return res.data.plant;
  },
  deletePlant: async (id: number): Promise<void> => {
    await api.delete(`/deletePlant/${id}`);
  },
  getPlantActions: async (plantId: number, page?: number, limit?: number,  type?: string): Promise<PaginatedResponse<PlantAction>> => {
    const res = await api.get<PaginatedResponse<PlantAction>>('/plantActions', {params: {plantId, page, limit, type}});
    console.log('plantsApi.getPlants response:', res);
    return res.data;
  },
  createPlantAction: async ( action: PlantActionRequest): Promise<PlantAction> => {
    const res = await api.post<PlantActionResponse>(`/createPlantAction`, action);
