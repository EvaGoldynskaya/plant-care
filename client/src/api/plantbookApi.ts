...
import type {
	PlantbookPlant,
	PlantbookSearchResponse,
} from "../types/plantbook.types"

const PLANTBOOK_API_URL = "https://open.plantbook.io/api/v1"
const PLANTBOOK_TOKEN = import.meta.env.VITE_PLANTBOOK_TOKEN
...
export const plantbookApi = {
	...
	getPlants: async (
		alias?: string,
		limit?: number
	): Promise<PlantbookSearchResponse> => {
		const res = await api.get<PlantbookSearchResponse>("/plant/search", {
			params: { alias, limit },
		})
		return res.data
	},
