import { message, Skeleton, Card, Empty, Button, Space, Descriptions } from "antd";
...
import { DropboxOutlined, HeatMapOutlined} from '@ant-design/icons';
...
const PlantDetailsPage = observer(() => {
  ...
  const { currentPlant: plant, plantIsLoading: isPlantLoading, currentPlantIsLoading: isActionsLoading } = plantStore;
  const { currentPlantbookPlant: plantbook, isLoadingPlantDetails: isPlantbookLoading } = plantbookStore;
      ...
      <Space orientation ="vertical" size="large" style={{ width: '100%' }}>
        ...
        <Card>
          <Descriptions column={3}>
              <Descriptions.Item label="Вид">{plant.commonName}</Descriptions.Item>
              {plantbook != null && (
                <>
                  <Descriptions.Item label="Семейство">{plantbook.category}</Descriptions.Item>
                  <Descriptions.Item label="Происхождение">{plantbook.origin}</Descriptions.Item>
                </>
              )}
          </Descriptions>
        </Card>
        {plantbook != null && (
          <Card>
            <Descriptions column={3}>
                <Descriptions.Item label='💡'> {plantbook.care?.light} </Descriptions.Item>
                <Descriptions.Item label='💧'>{plantbook.care?.humid}</Descriptions.Item>
                <Descriptions.Item label="🌱">{plantbook.care?.soil}</Descriptions.Item>
            </Descriptions>
          </Card>
        )}
        <div style={{ display: 'flex', gap: '0px', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: '0 0 20%' }}>
            <PlantActionButtons plantId={plant.id} onAction={handlePlantAction} />
          </div>
        </div>
      </Space>
    </div>
