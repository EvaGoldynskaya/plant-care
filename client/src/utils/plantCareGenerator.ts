...
const generateLightRecommendation = (data: PlantbookPlant): string => {
	...
	const avgLux =
		min_light_lux && max_light_lux ? (min_light_lux + max_light_lux) / 2 : null
	const avgMmol =
		min_light_mmol && max_light_mmol
			? (min_light_mmol + max_light_mmol) / 2
			: null
	...
	if (lightLevel !== null) {
		if (lightLevel < 500) {
			...
			placement =
				"располагать в затененных местах, вдали от прямых солнечных лучей"
			...
			placement =
				"размещать на хорошо освещенных местах, но защищать от полуденного солнца"
		} else {
			recommendation = "Очень светолюбивое растение"
			placement =
				"размещать на самом светлом месте, под прямыми солнечными лучами"
		}
	} else {
...
const generateHumidityRecommendation = (data: PlantbookPlant): string => {
	...
	const avgHumid =
		min_env_humid && max_env_humid ? (min_env_humid + max_env_humid) / 2 : null
	const avgSoilMoist =
		min_soil_moist && max_soil_moist
			? (min_soil_moist + max_soil_moist) / 2
			: null

	let airRecommendation: string
	let soilRecommendation: string
	...
	if (avgHumid !== null) {
		if (avgHumid < 40) {
			...
			airRecommendation =
				"Предпочитает высокую влажность воздуха, требует регулярного опрыскивания"
		}
	} else {
		airRecommendation = "Предпочитает умеренную влажность воздуха"
	}
	...
	if (avgSoilMoist !== null) {
		if (avgSoilMoist < 30) {
			soilRecommendation =
				"почва должна быть рыхлой и хорошо дренированной, полив умеренный, давать почве подсыхать"
		} else if (avgSoilMoist < 60) {
			soilRecommendation =
				"почва должна быть равномерно влажной, не допускать пересыхания или переувлажнения"
		} else {
			soilRecommendation =
				"почва должна быть постоянно влажной, не допускать пересыхания"
		}
	} else {
		soilRecommendation =
			"поддерживать умеренную влажность почвы, избегать застоя воды"
	}
...
const generateSoilRecommendation = (data: PlantbookPlant): string => {
	...
	if (avgMoist !== null) {
		if (avgMoist > 60) {
			soilRecommendation =
				"Предпочитает влагоемкую почву с высоким содержанием органики (торф, кокосовое волокно)"
		} else if (avgMoist < 30) {
			soilRecommendation =
				"Предпочитает легкую, хорошо дренированную почву с примесью песка или перлита"
		} else {
			soilRecommendation =
				"Предпочитает рыхлую, питательную почву с хорошим дренажем"
		}
	} else {
		soilRecommendation =
			"Предпочитает рыхлую, питательную почву с хорошим дренажем"
	...
	const avgEc =
		min_soil_ec && max_soil_ec ? (min_soil_ec + max_soil_ec) / 2 : null

	let ecAdvice = ""
	if (avgEc !== null) {
		if (avgEc < 500) {
			ecAdvice =
				"Удобряйте редко, используйте разбавленные растворы, чтобы не навредить."
		} else if (avgEc > 2000) {
			ecAdvice =
				"Удобряйте регулярно, но следите за концентрацией, не используйте слишком много удобрений."
		} else {
			ecAdvice =
				"Удобряйте регулярно в период активного роста, используя сбалансированные комплексные удобрения."
		}
	}
