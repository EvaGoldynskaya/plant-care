...
import type { Plant, PlantAction } from "../types/plant.types"
import type { PlantbookPlant } from "../types/plantbook.types"
...
describe("plantStore", () => {
			...
			data: [
				{
					...
					actions: [],
					plantbook: {} as unknown as PlantbookPlant,
				},
			],
			total: 1,
			page: 1,
	...
	it("sets current plant and loads actions when not already present", async () => {
		...
		vi.mocked(plantsApi.getPlantActions).mockResolvedValue({
			data: [
				{ id: 55, type: "ПОЛИВ", note: "Watered", createdAt: "2024-01-03" },
			],
			total: 1,
			page: 1,
			totalPages: 1,
		...
		expect(result).toEqual(
			expect.objectContaining({ id: 12, commonName: "Fern" })
		)
