...
import type { PlantbookPlant, PlantbookSearchPlant } from '../types/plantbook.types';
...
class PlantbookStore {
  ...
  searchPlants = async (query: string) => {
    ...
    try {
      ...
      runInAction(() => {
        this.searchError = error.message || 'Не удалось загрузить список растений';
        this.searchResults = [];
        this.isSearching = false;
      });
    }
  ...
  getPlantDetails = async (pid: string) => {
    ...
    try {
      ...
      runInAction(() => {
        this.plantDetailsError = error.message || 'Не удалось загрузить информацию о растении';
        this.currentPlantbookPlant = null;
        this.isLoadingPlantDetails = false;
      });
      return null;
