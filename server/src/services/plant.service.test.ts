...
describe("plant.service", () => {
  ...
  it("returns paginated plants for a user with search filter", async () => {
    mockedPrisma.plant.findMany.mockResolvedValue([{ id: 1, commonName: "Rose" }])
  ...
  it("returns a single plant for a user by id", async () => {
    mockedPrisma.plant.findUnique.mockResolvedValue({ id: 3, commonName: "Fern" })
  ...
  it("updates a plant and preserves existing values when fields are empty", async () => {
    ...
    mockedPrisma.plant.update.mockResolvedValue({ id: 4, commonName: "New name" })
  ...
  it("deletes a plant for the user", async () => {
    ...
    expect(mockedPrisma.plant.delete).toHaveBeenCalledWith({ where: { id: 22 } })
  })

  it("returns plant actions with filtering and pagination", async () => {
    mockedPrisma.plantAction.findMany.mockResolvedValue([{ id: 1, type: "water" }])
