...
export const getPlantActions = async (req: Request, res: Response) => {
	try {
		...
		const message =
			error instanceof Error ? error.message : "Failed to fetch plantActions"
		return res.status(400).json({ message })
	}
}
...
export const createPlantAction = async (req: Request, res: Response) => {
	try {
		...
		return res
			.status(200)
			.json({ message: "Plant action created", plantAction })
	} catch (error) {
		const message =
			error instanceof Error ? error.message : "Failed to create plant action"
		return res.status(400).json({ message })
	}
}
